...
import { buildWallFromLine, buildPlaneFromShape } from "@/helpers/create3dWall";
...
function updatePolygon(polygons) {
  ...
  if (wallMeshes) {
    for (let polygonMesh of wallMeshes) scene.value.remove(polygonMesh);
  }
...
onMounted(() => {
  ...
  const linePoints = [0, 5, 5, 5, 0, 0, 5, 0, 0, 0, 0, 5, 5, 0, 5, 5].map(
    (el) => el * 0.5
  );

  const points = [
    82.7578125, 128.109375, 359.7578125, 111.109375, 418.7578125, 350.109375,
    80.7578125, 347.109375, 89.7578125, 204.109375, 89.7578125, 204.109375,
    89.7578125, 204.109375, 82.7578125, 128.109375,
  ].map((el) => el * ratio);
...
const wallMeshesa = [];
const pointsForFloor = [];

// Build walls and collect points for the floor
for (let polygon of data) {
  const lines = splitArrayIntoChunks(polygon, 4);

  for (const line of lines) {
    let { wallMesh } = buildWallFromLine(scene.value, line);
    if (wallMesh) wallMeshesa.push(wallMesh);
  }

  // Collect all points for the floor
  pointsForFloor.push(...polygon);
}

// Build the floor plane with collected points
buildPlaneFromShape(scene.value, pointsForFloor);
